...
class ChordInstance(object):
    ...
    def to_dict(self):
        ...
        return {'ID':self.ID, 'IP':self.IP_ADDRESS, 'PORT':self.PORT,'Successor':self.successor,'Predecessor':self.predecessor}
...
if __name__ == '__main__':
    ...
    # server = SimpleXMLRPCServer.SimpleXMLRPCServer(('10.2.20.89', 9000))
    # server.register_instance(chord_instance())
    # server.serve_forever()

    client_ip = '10.2.24.78'
    client_port = 9001
    client = client.Server('http://{0}:{1}'.format(client_ip, client_port))
    chord_2 = ChordInstance('0.0.0.0',9002,1)
    dump = pickle.dumps(chord_2)
    print(dump)
    print(pickle.loads(dump))
    if (client.ping_back()):
        # print('hello world')
        # print('Successor: {0}, Predecessor: {1}'.format(client.get_successor(), client.get_predecessor()))
        # print(client.print_hello())
        #client.print_finger_table()
        print(client.get_successor())

        # print(client.get_ID())
        #print(client.print_finger_table())
        # client.set_successor(0, chord_2.to_dict())
        client.set_successor(2, pickle.dumps(chord_2))
        # client.set_ID(7)
        print(client.get_successor())
        # print(client.get_ID())
        #client.print_finger_table()
